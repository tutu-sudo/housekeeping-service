...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public Schedule createSchedule(Schedule schedule) {
        ...
        // 新业务逻辑：
        // - availableStatus = 1 表示“可服务时间”，需要做时间冲突校验；
        // - availableStatus = 0 表示“不可服务/请假时间”，允许覆盖，不做冲突校验。
        if (schedule.getAvailableStatus() == 1 &&
                hasTimeConflict(schedule.getStaffId(), schedule.getWorkDate(),
    ...
    public Schedule updateSchedule(Schedule schedule) {
        ...
        Integer newStatus = schedule.getAvailableStatus() != null
                ? schedule.getAvailableStatus()
                : db.getAvailableStatus();
        ...
        if (newStatus == 1 &&
                hasTimeConflict(staffId, workDate, start, end, db.getScheduleId())) {
            throw new BusinessException(ResultCode.SCHEDULE_TIME_CONFLICT);
        }
