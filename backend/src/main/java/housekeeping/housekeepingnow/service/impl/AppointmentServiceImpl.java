...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public void updateStatus(Long appointmentId, Integer status) {
        ...
        if (status == 4 && (appointment.getStatus() == 3 || appointment.getStatus() == 4)) {
            // 已完成或已取消的订单不能再次取消
    ...
    private void calculateTotalAmount(Appointment appointment) {
        ...
        switch (billingType) {
            case "daily":
                // 按天结算：价格 × 天数
                int days = appointment.getTotalDays() != null ? appointment.getTotalDays() : 1;
                totalAmount = servicePrice.multiply(BigDecimal.valueOf(days));
                break;
            case "times":
                // 按次数结算：价格 × 1次
            ...
            default:
                ...
                totalAmount = servicePrice.multiply(duration);
                break;
        }
