...
CREATE TABLE `appointment` (
    ...
    `customer_name` VARCHAR(50) DEFAULT NULL COMMENT '下单时填写的顾客姓名（冗余字段）',
    `customer_phone` VARCHAR(20) DEFAULT NULL COMMENT '下单时填写的顾客手机号（冗余字段）',
    `staff_id` BIGINT NOT NULL COMMENT '服务人员ID（关联staff表）',
    `service_id` BIGINT NOT NULL COMMENT '服务ID（关联service表）',
    ...
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '状态：0-待接单，1-已接单，2-服务中，3-已完成，4-已取消，5-已拒单/已关闭',
    `accept_time` DATETIME DEFAULT NULL COMMENT '服务人员接单时间',
    `paid_time` DATETIME DEFAULT NULL COMMENT '用户支付时间',
